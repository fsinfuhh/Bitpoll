...
def get_spam_label(spam_challenge):
    return "{} {} {} =".format(spam_challenge['x'], spam_challenge['op'], spam_challenge['y'])
...
def get_spam_challenge_from_key(key: str, poll_id: int) -> Dict:
    ...
    try:
        ...
        if spam_data.get('type') != 'anti_spam' or spam_data[
            'time'] <= time.time() - django_settings.ANTI_SPAM_CHALLENGE_TTL:
